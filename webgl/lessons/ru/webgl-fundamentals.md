...
Шейдеры необходимо компилировать (чтобы передать их на видеокарту) - для этого необходимо выполнить определенную последовательность вызовов, которая обработает исходный код шейдера. Данная последовательность универсальна для всех шейдеров, поэтому имеет смысл выделить ее в отдельную функцию.
Код шейдера - это обычный текст, а в современном JavaScript существуют некоторые неудобства при работе с многострочным текстом (которые, кстати, решены в ES6). Потому, для наглядности, напишем код шейдера внутри тэга script (с типом "notjs", конечно же), а после получим данный тэг по id и вытащим из него текст.
